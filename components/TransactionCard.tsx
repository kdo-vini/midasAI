...
export const TransactionCard: React.FC<TransactionCardProps> = ({
    ...
    const backgroundScale = useTransform(x, [0, 50], [0.8, 1]);

    const handleDragEnd = (_: any, info: PanInfo) => {
        if (!isFixedExpense) return;

        // Swipe right to toggle paid
        if (info.offset.x > 80) {
    ...
    return (
        <div className="relative overflow-hidden rounded-lg">
            {/* Swipe Background Action (Only for Fixed Expenses) */}
            {isFixedExpense && (
                <motion.div
            ...
            <motion.div
                drag={isFixedExpense ? "x" : false}
                dragConstraints={{ left: 0, right: 0 }}
                dragElastic={{ left: 0, right: 0.5 }}
                onDragEnd={handleDragEnd}
                style={{ x }}
                className={`${getBackgroundColor()} ${getBorderColor()} border rounded-lg p-4 transition-colors duration-200 relative bg-white dark:bg-slate-800 z-10`}
            >
